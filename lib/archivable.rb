module Archivable
  ...
  def modified
    return (updated_at.present? ? updated_at.strftime(:simple_time) : '') unless self.respond_to?(:updated_by_id)
    user = User.find_by_id(updated_by_id)
    user.nil? ? "unknown" : "#{user.first_name[0]}#{user.last_name}@#{updated_at.strftime(:simple_time)}"
  end
